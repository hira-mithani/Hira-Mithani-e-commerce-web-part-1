...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get(`https://fakestoreapi.com/products/${param?.productId}`);
            ...
            <Grid item xs={12} sm={6} sx={{
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  <Rating value={ProductDetails?.rating?.rate || 0} readOnly sx={{
                    marginRight: '1rem',
                    color: '#FFC107',
